...
- 下一次进入发布界面时会出现上一次保存的草稿
  ...
  点击加号可选择本地相册中的图片，一次最多选择9张，在满九张后加号会消失，不可选择图片，同时发布的最多也是9张。、
...
这次的大作业整体难度不高，但很好的结合了**数据存储**与**`block`回调**的知识点。除此之外，让我借此机会学习并使用一些别的功能，类如图片的保存与获取，WCDB数据存储，还有`YYText`框架的初步使用，对细节的打磨，让我之前忘的差不多的动画也有机会展现。
